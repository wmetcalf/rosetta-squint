...
final class Group2TiffTests: XCTestCase {
    func testByteExactAllTiff() throws {
        let fixtures = try TestKit.listValidFixtures("tiff")
        XCTAssertEqual(fixtures.count, 10, "expected 10 TIFF fixtures")
        ...
        if !failures.isEmpty {
            XCTFail("\(failures.count) TIFF failures:\n  \(failures.joined(separator: "\n  "))")
        }
    }
}
