...
final class Group2GifTests: XCTestCase {
    func testByteExactAllGif() throws {
        let fixtures = try TestKit.listValidFixtures("gif")
        XCTAssertFalse(fixtures.isEmpty, "should have GIF fixtures")
        ...
        for rel in fixtures {
            ...
            do {
                ...
                if got.width != want.width || got.height != want.height || got.channels.bytesPerPixel != want.channels {
                    failures.append("\(rel): shape \(got.width)x\(got.height)c\(got.channels.bytesPerPixel) != \(want.width)x\(want.height)c\(want.channels)")
                    continue
                }
        ...
        if !failures.isEmpty {
            XCTFail("\(failures.count) GIF failures:\n  \(failures.joined(separator: "\n  "))")
        }
    }
}
