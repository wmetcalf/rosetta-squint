...
final class Group2WebpTests: XCTestCase {
    func testByteExactAllWebp() throws {
        let fixtures = try TestKit.listValidFixtures("webp")
        XCTAssertEqual(fixtures.count, 12, "expected 12 WebP fixtures")
        ...
        if !failures.isEmpty {
            XCTFail("\(failures.count) WebP failures:\n  \(failures.joined(separator: "\n  "))")
        }
    }
}
