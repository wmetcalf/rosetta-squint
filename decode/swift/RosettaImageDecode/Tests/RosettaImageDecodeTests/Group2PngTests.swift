...
final class Group2PngTests: XCTestCase {
    func testByteExactAllPng() throws {
        let fixtures = try TestKit.listValidFixtures("png")
        XCTAssertFalse(fixtures.isEmpty, "should have PNG fixtures")
        ...
        if !failures.isEmpty {
            XCTFail("\(failures.count) PNG failures:\n  \(failures.joined(separator: "\n  "))")
        }
    }
}
