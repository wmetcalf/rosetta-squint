...
extension Group4ErrorTests {
    func testInvalidJpegFixtures() throws {
        let errors = try TestKit.readErrors()
        var failures: [String] = []
        for (key, expected) in errors {
            guard key.hasPrefix("jpeg/") else { continue }
            let input = try TestKit.readFixture(key)
            do {
                _ = try Decoder.decode(input)
                failures.append("\(key): decode succeeded, expected \(expected.expected_kind)")
        ...
        if !failures.isEmpty {
            XCTFail("\(failures.count) Group-4 JPEG failures:\n  \(failures.joined(separator: "\n  "))")
        }
    }
}

extension Group4ErrorTests {
    func testInvalidWebpFixtures() throws {
        let errors = try TestKit.readErrors()
        var failures: [String] = []
        for (key, expected) in errors {
            guard key.hasPrefix("webp/") else { continue }
            let input = try TestKit.readFixture(key)
            do {
                _ = try Decoder.decode(input)
                failures.append("\(key): decode succeeded, expected \(expected.expected_kind)")
        ...
        if !failures.isEmpty {
            XCTFail("\(failures.count) Group-4 WebP failures:\n  \(failures.joined(separator: "\n  "))")
        }
    }
}

extension Group4ErrorTests {
    func testInvalidTiffFixtures() throws {
        let errors = try TestKit.readErrors()
        var failures: [String] = []
        for (key, expected) in errors {
            guard key.hasPrefix("tiff/") else { continue }
            let input = try TestKit.readFixture(key)
            do {
                _ = try Decoder.decode(input)
                failures.append("\(key): decode succeeded, expected \(expected.expected_kind)")
        ...
        if !failures.isEmpty {
            XCTFail("\(failures.count) Group-4 TIFF failures:\n  \(failures.joined(separator: "\n  "))")
        }
    }
}
