...
public class Group2TiffGoldensTests {
    @Test
    public void byteExactAllTiff() throws IOException {
        List<String> fixtures = TestKit.listValidFixtures("tiff");
        assertEquals(10, fixtures.size(), "expected 10 TIFF fixtures");
                ...
                for (int i = 0; i < gotData.length; i++) {
                    if (gotData[i] != want.pixels()[i]) {
                        failures.add(rel + ": pixel byte " + i
                                + " got=" + (gotData[i] & 0xFF) + " want=" + (want.pixels()[i] & 0xFF));
                        break;
                    }
                }
        ...
        if (!failures.isEmpty()) {
            fail(failures.size() + " TIFF byte-exact failures:\n  " + String.join("\n  ", failures));
        }
    }
}
