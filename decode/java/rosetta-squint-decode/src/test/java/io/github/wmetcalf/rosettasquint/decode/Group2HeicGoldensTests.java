...
public class Group2HeicGoldensTests {
    @Test
    public void byteExactAllHeic() throws IOException {
        List<String> fixtures = TestKit.listValidFixtures("heic");
        assertEquals(10, fixtures.size(), "expected 10 HEIC fixtures");
        ...
        if (!failures.isEmpty()) {
            fail(failures.size() + " HEIC byte-exact failures:\n  " + String.join("\n  ", failures));
        }
    }
}
