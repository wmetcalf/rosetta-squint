# rosetta-squint-decode — Java port
...
PNG and GIF use `javax.imageio` from the JDK. The system-scope `turbojpeg.jar` is from the Ubuntu package `libturbojpeg-java` (`/usr/share/java/turbojpeg.jar`). HEIC uses a hand-written JNA wrapper around `libheif.so.1` (in `io.github.wmetcalf.rosettasquint.decode.internal.libheif`).
