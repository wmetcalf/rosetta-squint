...
func TestGroup4InvalidWebpFixtures(t *testing.T) {
	errs := readErrors(t)
	var failures []string
	for key, expected := range errs {
		if !strings.HasPrefix(key, "webp/") {
			continue
		}
		input := readFixture(t, key)
		_, err := Decode(input)
	...
	if len(failures) > 0 {
		t.Fatalf("%d Group-4 WebP failures:\n  %s", len(failures), strings.Join(failures, "\n  "))
	}
}

func TestGroup4InvalidTiffFixtures(t *testing.T) {
	errs := readErrors(t)
	var failures []string
	for key, expected := range errs {
		if !strings.HasPrefix(key, "tiff/") {
			continue
		}
		input := readFixture(t, key)
		_, err := Decode(input)
	...
	if len(failures) > 0 {
		t.Fatalf("%d Group-4 TIFF failures:\n  %s", len(failures), strings.Join(failures, "\n  "))
	}
}

func TestGroup4InvalidHeicFixtures(t *testing.T) {
	errs := readErrors(t)
	var failures []string
	for key, expected := range errs {
		if !strings.HasPrefix(key, "heic/") {
			continue
		}
		input := readFixture(t, key)
		_, err := Decode(input)
	...
	if len(failures) > 0 {
		t.Fatalf("%d Group-4 HEIC failures:\n  %s", len(failures), strings.Join(failures, "\n  "))
	}
}
