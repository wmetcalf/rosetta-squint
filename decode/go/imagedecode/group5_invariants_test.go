...
func TestGroup5SupportedFormatsBmpPngGifJpegWebpTiff(t *testing.T) {
	...
	if len(supported) != 7 {
		t.Errorf("expected 7 supported formats, got %d", len(supported))
	}
	hasBmp, hasPng, hasGif, hasJpeg, hasWebp, hasTiff, hasHeic := false, false, false, false, false, false, false
	...
	if !hasBmp || !hasPng || !hasGif || !hasJpeg || !hasWebp || !hasTiff || !hasHeic {
		t.Errorf("expected Bmp + Png + Gif + Jpeg + Webp + Tiff + Heic in supportedFormats, got %v", supported)
	}
}
...
func TestGroup5AllDecodedHeicImagesHaveValidShape(t *testing.T) {
	...
	for _, rel := range fixtures {
		...
		if img.Format != Heic {
			t.Errorf("%s: format should be Heic, got %v", rel, img.Format)
		}
		expectedBytes := img.Width * img.Height * img.Channels.BytesPerPixel()
		if len(img.Data) != expectedBytes {
			t.Errorf("%s: data length %d != expected %d", rel, len(img.Data), expectedBytes)
