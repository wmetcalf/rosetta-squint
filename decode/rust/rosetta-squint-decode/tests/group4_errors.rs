...
#[test]
fn invalid_webp_fixtures() {
    let errors = testkit::read_errors();
    let mut failures: Vec<String> = Vec::new();
    for (key, expected) in errors.iter() {
        if !key.starts_with("webp/") {
            continue;
        }
        let input = testkit::read_fixture(key);
        match decode(&input) {
...
#[test]
fn invalid_tiff_fixtures() {
    let errors = testkit::read_errors();
    let mut failures: Vec<String> = Vec::new();
    for (key, expected) in errors.iter() {
        if !key.starts_with("tiff/") {
            continue;
        }
        let input = testkit::read_fixture(key);
        match decode(&input) {
...
#[test]
fn invalid_heic_fixtures() {
    let errors = testkit::read_errors();
    let mut failures: Vec<String> = Vec::new();
    for (key, expected) in errors.iter() {
        if !key.starts_with("heic/") {
            continue;
        }
        let input = testkit::read_fixture(key);
        match decode(&input) {
