...
describe("Group 2 — byte-exact goldens (WebP)", () => {
  it("matches all WebP fixtures byte-exact", async () => {
    const fixtures = listValidFixtures("webp");
    if (fixtures.length === 0) throw new Error("no WebP fixtures");
    ...
    if (failures.length > 0) {
      throw new Error(`${failures.length} WebP failures:\n  ${failures.join("\n  ")}`);
    }
  });
});
