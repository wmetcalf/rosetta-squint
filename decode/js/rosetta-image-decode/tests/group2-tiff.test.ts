...
describe("Group 2 — byte-exact goldens (TIFF)", () => {
  it("matches all TIFF fixtures byte-exact", async () => {
    const fixtures = listValidFixtures("tiff");
    if (fixtures.length === 0) throw new Error("no TIFF fixtures");
    ...
    if (failures.length > 0) {
      throw new Error(`${failures.length} TIFF failures:\n  ${failures.join("\n  ")}`);
    }
  });
});
