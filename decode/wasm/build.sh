#!/usr/bin/env bash
# Build the portable, byte-exact libheif+libde265 WASM decoder for rosetta-squint.
#
# Produces libheif_decode.wasm: a NON-shared-memory WASI reactor module that
# decodes HEIC to interleaved RGB/RGBA, BIT-IDENTICAL to a native libheif 1.21.2
# decode and deterministic across wasm runtimes. Because it uses no threads /
# shared memory, it runs in EVERY runtime with no special features: wazero (Go),
# V8 (JS), wasmtime (Rust), Chicory (Java), WasmKit (Swift) — all verified
# byte-exact. One artifact, every port -> identical HEIC pixels -> identical hash.
#
...
ROOT="${1:-$HOME/heic-wasm-ns}"        # work dir
...
# 2. patches
#  - libde265: Threads from wasi-libc, not a separate lib (drop find_package REQUIRED + link)
#  - libheif:  __wasi__ branch in decoder_libde265.cc -> synchronous decode, filters ON
...
# NON-pthread toolchain: no shared memory. libde265's thread pool uses pthread
# (emulated stubs, never started); de265.cc's std::mutex init-guard needs the
# std_mutex_shim (the no-pthread libc++ omits <mutex>).
TC="$WASI/share/cmake/wasi-sdk.cmake"
EMU="-D_WASI_EMULATED_SIGNAL -D_WASI_EMULATED_MMAN -D_WASI_EMULATED_PROCESS_CLOCKS -D_WASI_EMULATED_GETPID -D_WASI_EMULATED_PTHREAD"
CXXSHIM="-include $HERE/shim/std_mutex_shim.h"
...
cmake -S libde265 -B build-de265 -G Ninja -DCMAKE_TOOLCHAIN_FILE="$TC" \
  ...
  -DCMAKE_C_FLAGS="$EMU" -DCMAKE_CXX_FLAGS="$EMU $CXXSHIM" -DCMAKE_BUILD_TYPE=Release
...
cmake -S libheif -B build-heif -G Ninja -DCMAKE_TOOLCHAIN_FILE="$TC" \
  ...
  -DCMAKE_CXX_FLAGS="$EMU $CXXSHIM -include $HERE/shim/heif_shim.h" -DCMAKE_BUILD_TYPE=Release
cmake --build build-heif --target heif -j"$(nproc)"

# 5. shim (mkstemp + no-exception __cxa stubs) and final NON-shared reactor link
"$WASI/bin/clang" --target=wasm32-wasi --sysroot="$WASI/share/wasi-sysroot" -O2 \
...
"$WASI/bin/clang++" --target=wasm32-wasi --sysroot="$WASI/share/wasi-sysroot" \
  ...
  heif_shim.o -lwasi-emulated-signal -lwasi-emulated-mman -lwasi-emulated-process-clocks \
  -lwasi-emulated-getpid -lwasi-emulated-pthread \
  $EXPFLAGS -o libheif_decode.wasm
echo "built: $ROOT/libheif_decode.wasm (imports only wasi_snapshot_preview1; no shared memory)"
...
#   heif_decode_image(err,h,&img, 1/*RGB*/, 10|11/*interleaved RGB|RGBA*/, 0);
#   heif_image_get_plane_readonly(img,10/*interleaved*/,&stride); de-stride to packed RGB(A).
