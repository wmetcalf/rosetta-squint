...
use rosetta_squint_hash::{average_hash, crop_resistant_hash, dhash_vertical, phash_simple, whash_db4, whash_db4_robust};
...
fn dhash_goldens() {
	...
	for c in &cases {
		let img = load_predecoded(&c.fixture);
		let h = rosetta_squint_hash::dhash(&img, c.size).expect("compute");
		if h.to_hex() != c.hex {
...
fn phash_goldens() {
    ...
    for c in &cases {
        let img = load_predecoded(&c.fixture);
        let h = rosetta_squint_hash::phash(&img, c.size).expect("compute");
        if h.to_hex() != c.hex {
            failures.push(format!("fixture={} size={} got={} want={}", c.fixture, c.size, h.to_hex(), c.hex));
        }
    }
...
fn whash_haar_goldens() {
    ...
    for c in &cases {
        let img = load_predecoded(&c.fixture);
        let h = rosetta_squint_hash::whash_haar(&img, c.size).expect("compute");
        if h.to_hex() != c.hex {
            failures.push(format!("fixture={} size={} got={} want={}", c.fixture, c.size, h.to_hex(), c.hex));
        }
    }
...
fn colorhash_goldens() {
    ...
    for c in &cases {
        let img = load_predecoded(&c.fixture);
        let h = rosetta_squint_hash::colorhash(&img, c.size).expect("compute");
        if h.to_hex() != c.hex {
...
#[test]
fn bin_encoding_b4() {
    use rosetta_squint_hash::colorhash_bin_encode;
