...
**3.1 PIL `GaussianBlur(radius=2)`:** Pillow approximates a 2-D gaussian via 3 separable box filters (horizontal pass then transpose for vertical pass, applied 3 times). The box radius is **NOT** simply `radius`. Pillow computes it via the Gwosdek 2011 formula `_gaussian_blur_radius(std_dev=2.0, n_passes=3)` which yields `box_radius ≈ 1.375` for `radius=2`. The fractional part forces a fixed-point weighting scheme (`ww`/`fw` constants in `BoxBlur.c`) where the integer-radius pixels get full weight and the fractional-edge pixel gets a weight of `fw` (with the integer pixels using `ww`). The Rust port's `src/internal/pil_gaussian_blur.rs` is a working reference port; consult Pillow's upstream `src/libImaging/BoxBlur.c` for the canonical algorithm. Boundary mode is edge replication. Reference vectors in `spec/gaussian_blur_cases.json`.
...
**3.4 Bounding-box float→int rounding:** PIL's `Image.crop((left, top, right, bottom))` accepts floats and internally calls `map(int, map(round, box))` — i.e. **round-half-to-even THEN cast to int** (NOT plain truncate toward zero as an earlier draft of this spec incorrectly stated). The Rust port's `src/crop_resistant.rs` uses this rounding to match Python. Other ports must do likewise: e.g. Java `Math.round` (which is round-half-up — NOT exactly the same as Python's banker's rounding, but works for the .5 ambiguity if you instead use `Math.rint` which IS round-half-to-even); Go `math.RoundToEven`; JS `Math.round` is round-half-away-from-zero (problematic — use a banker's-rounding helper); Swift `.rounded(.toNearestOrEven)`.
