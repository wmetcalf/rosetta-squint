...
class DHashVerticalGoldenTest {
    @TestFactory
    List<DynamicTest> dhashVerticalMatchesGoldens() {
        List<DynamicTest> tests = new ArrayList<>();
        for (Goldens.Triple t : Goldens.algorithmCases("dhash_vertical")) {
            tests.add(dynamicTest("dhash_vertical " + t.fixture() + " size=" + t.size(), () -> {
                BufferedImage img = PreDecoded.loadAsBufferedImage(t.fixture());
                ImageHash result = DHashVertical.compute(img, t.size());
