...
import io.github.wmetcalf.rosettasquint.hash.internal.PilGaussianBlur;
    ...
    List<DynamicTest> allCasesMatchSpec() throws IOException {
        ...
        for (Iterator<JsonNode> it = cases.elements(); it.hasNext(); ) {
            ...
            tests.add(dynamicTest("gaussian_blur " + name, () -> {
                ...
                int[][] actual = PilGaussianBlur.apply(input);
                for (int y = 0; y < H; y++) {
                    for (int x = 0; x < W; x++) {
                        int ey = y, ex = x;
