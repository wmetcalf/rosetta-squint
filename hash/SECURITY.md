...
The decoders (when present: `decodePng`, `decodePNG`) reject malformed PNG signatures and propagate errors from the underlying PNG library (`image::ImageReader` Rust, `pngjs` JS, swift-png). They do **not** apply a MAX_PIXELS cap — the hash library is intended for already-trusted RGB input. **If you're hashing untrusted images, decode them with [rosetta-image-decode](../rosetta-image-decode) first**, which does enforce MAX_PIXELS.
